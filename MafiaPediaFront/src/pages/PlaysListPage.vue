<template>
  <div dir="rtl" class="max-w-4xl mx-auto px-6 w-full">
    <h1 class="text-2xl md:text-3xl font-bold mb-6 text-fg">لیست بازی‌ها</h1>
    ...
    <div class="bg-surface rounded-[10px] border border-border p-4 mb-6">
      <div class="flex flex-wrap items-end gap-3">
        ...
        <button
          class="px-3 py-1.5 rounded text-sm border border-border text-muted hover:bg-surface-hover transition"
          @click="clearFilters"
        >
          پاک کردن فیلترها
        </button>
          ...
          >
            ...
            <td class="px-4 py-3">
              <div class="flex items-center gap-3">
                ...
                <span class="font-medium">{{ play.title }}</span>
              </div>
            </td>
            <td class="px-4 py-3 text-muted whitespace-nowrap" dir="ltr">{{ formatDate(play.dateTime) }}</td>
            ...
            <td class="px-4 py-3 text-muted">{{ play.eventName }}</td>
            <td class="px-4 py-3">
              <a
                v-if="play.link"
                :href="play.link"
...
async function fetchPlays() {
  loading.value = true
  try {
    const res = await PlaysApi.getPlays(page.value, pageSize, searchQuery.value || undefined, buildFilterParams())
