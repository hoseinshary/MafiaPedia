<template>
  <div dir="rtl" class="max-w-lg mx-auto w-full">
    <div class="mb-8">
      <h1 class="text-xl font-bold text-gold-text">پروفایل</h1>
      <p class="text-sm text-muted mt-1">تنظیمات حساب کاربری</p>
    </div>
    ...
    <template v-else>
      ...
      <div class="bg-surface border border-border rounded-xl p-6 mb-6">
        <div class="flex flex-col gap-4">
          <div class="flex flex-col gap-1">
            ...
            <input v-model="form.username" type="text" class="bg-[var(--color-bg)] border border-border rounded px-3 py-2 text-sm text-fg placeholder-muted focus:outline-none focus:border-gold transition" />
          </div>
          <div class="flex flex-col gap-1">
            <label class="text-sm text-muted">نام نمایشی</label>
            <input v-model="form.displayName" type="text" class="bg-[var(--color-bg)] border border-border rounded px-3 py-2 text-sm text-fg placeholder-muted focus:outline-none focus:border-gold transition" />
          </div>
          <div class="flex flex-col gap-1">
            <label class="text-sm text-muted">شماره موبایل</label>
            <input :value="account?.mobile" type="text" dir="ltr" disabled class="bg-[var(--color-bg)] border border-border rounded px-3 py-2 text-sm text-muted opacity-60 cursor-not-allowed" />
            <span class="text-xs text-muted mt-1">شماره موبایل قابل ویرایش نیست</span>
          </div>
          <button @click="handleSubmit" :disabled="submitting" class="self-start px-5 py-2 bg-gold text-[#141416] text-sm font-bold rounded hover:opacity-90 transition disabled:opacity-50 disabled:cursor-not-allowed inline-flex items-center gap-2">
      ...
      <div v-if="hasAnyLinked" class="space-y-4">
        <div v-if="account?.player" class="bg-surface border border-border rounded-xl p-6">
          <h3 class="text-sm font-bold text-fg mb-4">پروفایل عمومی بازیکن (فاز ۱)</h3>
          <LinkedPictureSection :name="account.player.name" :picture="account.player.picture" @upload="handleUpload('player', $event)" />
        </div>
        <div v-if="account?.clubplayer" class="bg-surface border border-border rounded-xl p-6">
          <h3 class="text-sm font-bold text-fg mb-4">پروفایل مشتری کافه</h3>
        ...
        <div v-if="account?.master" class="bg-surface border border-border rounded-xl p-6">
          <h3 class="text-sm font-bold text-fg mb-4">پروفایل گرداننده</h3>
          <LinkedPictureSection :name="account.master.name" :picture="account.master.photo" @upload="handleUpload('master', $event)" />
        </div>
      </div>
      <div v-else class="bg-surface border border-border rounded-xl p-6 text-center">
        <p class="text-sm text-muted">هیچ رکورد لینک‌شده‌ای یافت نشد</p>
      </div>
    </template>
  </div>
</template>
