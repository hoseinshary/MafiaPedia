<template>
  <Teleport to="body">
    <div v-if="isOpen" dir="rtl" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div class="relative bg-gray-800 rounded-lg w-[90%] sm:w-full p-6 shadow-xl border border-gray-700 max-h-[90vh] overflow-y-auto" :class="maxWidthClass">
        ...
        <slot />
        <div v-if="$slots.footer" class="flex gap-3 justify-end mt-6 pt-4 border-t border-gray-700">
          <slot name="footer" />
        </div>
      </div>
    </div>
