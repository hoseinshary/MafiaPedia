...
[ApiController]
[Route("api/club/product-category")]
...
public class ProductCategoryController : ControllerBase
{
    ...
    public ProductCategoryController(IProductCategoryService service, IClubUserService clubUserService)
    {
        _service = service;
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromQuery] int clubId, [FromBody] CreateProductCategoryDto dto)
    {
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, [FromQuery] int clubId, [FromBody] UpdateProductCategoryDto dto)
    {
        await VerifyAccessAsync(clubId, "owner", "cashier");
        var result = await _service.UpdateAsync(clubId, id, dto);
        return Ok(result);
    }
