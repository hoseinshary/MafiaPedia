...
public class ProductController : ControllerBase
{
    ...
    public ProductController(IProductService service, IClubUserService clubUserService)
    {
        _service = service;
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromQuery] int clubId, [FromBody] CreateProductDto dto)
    {
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, [FromQuery] int clubId, [FromBody] UpdateProductDto dto)
    {
        await VerifyAccessAsync(clubId, "owner", "cashier");
        var userId = int.Parse(User.FindFirstValue(ClaimTypes.NameIdentifier)!);
